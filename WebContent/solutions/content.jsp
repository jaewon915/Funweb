...
<a href="../upload/<%=pb.getFile()%>"><%=pb.getFile()%></a>
</td></tr>
<tr><td>글내용</td><td colspan=3><%=pb.getContent()%></td></tr>
<tr><td colspan=4>
<input type="button" value="글수정" onclick="location.href='updateForm.jsp?num=<%=pb.getNum()%>&pageNum=<%=pageNum%>'">
<input type="button" value="글삭제" onclick="location.href='deleteForm.jsp?num=<%=pb.getNum()%>&pageNum=<%=pageNum%>'">
