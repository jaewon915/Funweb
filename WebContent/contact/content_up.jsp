...
<%
	for(int i=0; i < board2List.size(); i++){
		...
		//if(b2b.getNum() == )		
%>
	<tr><td><%=b2b.getNum2() %></td>
	
	
<form>
<td class="left"><input type="text" name="content2" value="<%= b2b.getContent2()%>"></td>
</form>


<td><%= b2b.getName2()%><a href="content_up.jsp?num=<%=drb.getNum()%>&pageNum=<%=pageNum%>">[수정]</a>[삭제]</td><td><%=b2b.getDate2() %></td>
