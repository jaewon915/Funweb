...
<%
if(id!=null){
	if(id.equals(b2b.getName2()) ){ //로그인 = 글쓴이 같으면 글수정, 글삭제 보임
%>

<input type="button" value="수정" onclick="location.href='content_upForm.jsp?num=<%=drb.getNum()%>&num2=<%=b2b.getNum2()%>&pageNum=<%=pageNum%>'">
<input type="button" value="삭제" onclick="location.href='content_deForm.jsp?num=<%=drb.getNum()%>&num2=<%=b2b.getNum2()%>&pageNum=<%=pageNum%>'">
<%
	}
}
%>
